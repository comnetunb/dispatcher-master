...
export class FileUploadComponent implements OnInit {
  ...
  success(event) {
    if (!(event.target && event.target.files)
      ...
      && !(event.mouseEvent && event.mouseEvent.dataTransfer && event.mouseEvent.dataTransfer.files)) {
    ...
    for (let file of files) {
      ...
      let request = this.filesService.upload(body).subscribe(event => {
        if (event.type === HttpEventType.UploadProgress) {
          fileForm.controls['progress'].setValue(Math.round(100 * event.loaded / event.total));
